...
class _GameState extends State<Game> {
  ...
  Future<void> _initializeBoard() async {
    ...
    if (gameDifficulty == 0x7FFFFFFFFFFFFFFF) gameDifficulty = 0;
    ...
    return Column(
      ...
      children: [
        for (int i = 0; i < subtitles.length; i++) ...[
          buildSectionHeader(context, subtitles[i]),
        ...
        builder: (context, constraints) {
          boardSize =
              constraints.maxWidth < boardMaxSize
                  ...
                  : boardMaxSize;
          return isTall ? _buildTallLayout() : _buildShortLayout();
        },
      ),
    );
  }
  ...
  Widget _buildTallLayout() {
    return Column(
      children: [
        ...
        const SizedBox(height: 12),
        _buildHeaderText('Razina: ', '${gameDifficulty + 1}'),
        const SizedBox(height: 12),
        _buildHeaderText(
          'Trenutni cilj: ',
          goal.checkpoints!.isNotEmpty
              ? toChessCoords(
                goal.checkpoints!.first.pair.x,
                goal.checkpoints!.first.pair.y,
              )
              : ' ',
        ),
        const SizedBox(height: 24),
  ...
  Widget _buildHeaderText(String label, String value) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      width: boardSize,
        ...
        text: TextSpan(
          ...
          children: [
            ...
            TextSpan(
              text: value,
              style: TextStyle(
                fontSize: Styles.headerLarge(context).fontSize! * 1.4,
                fontWeight: FontWeight.bold,
